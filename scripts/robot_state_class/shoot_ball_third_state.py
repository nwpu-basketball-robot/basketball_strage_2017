...
class Shoot(smach.State):
    ...
    def execute(self, ud):
        ...
        self.move_cmd.move_to(math.tan(cylinder_theta)*cylinder_dis+0.15,0,0.5)
        (cylinder_dis,cylinder_theta) = self.find_cylinder.findcylinder_ni() 
...
class Find_Another_Ball(smach.State):
    ...
    def execute(self, ud):
        ...
        self.move_cmd.move_to(x=9.4-current_x, y=0, max_velocity=1.2)
        current_y = self.cmd_position.get_robot_current_y()
        self.move_cmd.move_to(x=0.0,y=1.35-current_y,max_velocity=1.2)
        current_w = self.cmd_position.get_robot_current_w()
        self.turn_cmd.turn_to(-current_w+math.pi/1.9)
        return 'successed'
