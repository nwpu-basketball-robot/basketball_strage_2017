...
class Search_Ball_Shun(smach.State):
    def __init__(self):
        ...
        self.find_basketball = find_basketball.find_basketball()
    ...
    def execute(self, ud):
        ...
        if self.preempt_requested():
            self.service_preempt()
            return 'failed'
        (ball_dis,ball_theta) = self.find_basketball.findball_shun()
        self.move_cmd.move_to(math.tan(ball_theta)*ball_dis+0.03,0,0.5)
        (ball_dis,ball_theta) = self.find_basketball.findball_shun()
        #(ball_dis,ball_theta) = self.find_basketball.findball_directly()
        if abs(ball_dis)>1.3:
...
class Search_Ball_Ni(smach.State):
    ...
    def execute(self, ud):
        ...
        if self.preempt_requested():
            self.service_preempt()
            return 'failed'
        (ball_dis,ball_theta) = self.find_basketball.findball_ni()
        self.move_cmd.move_to(math.tan(ball_theta)*ball_dis+0.03,0,0.5)
        (ball_dis,ball_theta) = self.find_basketball.findball_ni()
        #(ball_dis,ball_theta) = self.find_basketball.findball_directly()
        if abs(ball_dis)>1.3:
...
class Shoot_Adjust1(smach.State):
    ...
    def execute(self,ud):
        ...
        self.move_cmd.move_to(x=1.1-current_x, y=0,max_velocity = 1)
        current_y = self.cmd_position.get_robot_current_y()
        self.move_cmd.move_to(x=0, y=7.1-current_y,max_velocity = 1)
        current_w = self.cmd_position.get_robot_current_w()
        self.turn_cmd.turn_to(-current_w+math.pi/2.5)
        self.cmd_shovel.control_shovel(control_type = 4)
        return 'successed'
...
class Find_Another_Ball(smach.State):
    ...
    def execute(self, ud):
        ...
        current_x = self.cmd_position.get_robot_current_x()
        self.move_cmd.move_to(x=1.4-current_x, y=0,max_velocity = 1)
        current_w = self.cmd_position.get_robot_current_w()
...
class Return(smach.State):
    ...
    def execute(self,ud):
        if self.preempt_requested():
            ...
            return 'failed' 
        current_y = self.cmd_position.get_robot_current_y()   
        self.move_cmd.move_to(x = 0, y = 3-current_y ,max_velocity = 1)
        (current_x,current_y) = self.cmd_position.get_robot_current_x_y()
        rospy.logwarn('current_x=%s,current_y=%s'%(current_x,current_y))
